// @ts-nocheck
import * as ChunkType from './util/chunks';
...
DFFReader.prototype = {
   ...
   readString: function(length) {
      ...
      while (this.position < end) {
         const val = this.data.getUint8(this.position++, true);
         if (val == 0) {
            this.position = end;
            break;
         }
   ...
   readChunk: function (type, parent) {
      ...
      const readHeader = this.readHeader(parent);

      if (type != readHeader.type) {
         ...
         this.position += readHeader.length;
         return null;
      }

      position = this.position;
      const chunk = this.readData(readHeader);

      if (this.position < position + readHeader.length) {
         console.warn('DFFLoader: Chunk ' + readHeader.name + ' not readed to end');
         this.position = position + readHeader.length;
      } else if (this.position > position + readHeader.length) {
         throw (
            'DFFLoader: Offset is outside the bounds of the chunk ' + readHeader.name
         );
      }
      return chunk;
   },
